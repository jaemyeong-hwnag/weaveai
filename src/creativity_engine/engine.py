...
class CreativityEngine:
    ...
    config를 통해 LLM 클라이언트, 활성 제너레이터, 스코어 가중치 등
    서비스별 동작을 완전히 교체할 수 있음.

    사용법:
        # 기본 사용
        engine = CreativityEngine()
        result = engine.run("원격 근무자의 집중력 저하 문제")
        ...
        engine = CreativityEngine(config=config, adapter=LegalAdapter())
    """
    ...
    def __init__(
        ...
        self._input_engine = InputEngine(self._config)
        self._connection_engine = ConnectionEngine(self._config)
        self._output_engine = OutputEngine(self._config)

    def run(self, raw: dict | str, **kwargs) -> Any:
        """
        6단계 파이프라인 실행 후 도메인 포맷으로 렌더링된 결과 반환.
        kwargs로 divergence_n, top_k, serendipity 등을 오버라이드 가능.
        """
        if kwargs:
    ...
    def run_raw(self, raw: dict | str, **kwargs) -> Solution:
        """render_solution 없이 Solution 객체 그대로 반환. 디버깅·테스트용."""
        if kwargs:
            if isinstance(raw, str):
                raw = {"goal": raw, **kwargs}
            else:
